...
import { createData, deleteData, getAll, updateData } from '@/api/regionAreaService'
import Vue from "vue";
import {mapActions, mapGetters} from "vuex";
import FormRegion from "@/views/dashboard/pages/RegionArea/components/FormRegion.vue";

export default {
  name: "MasterRegion",
  components: {FormRegion, ConfirmDeleteDialog },
  data() {
    return {
      snackbar: {
        open : false,
  ...
  methods: {
    ...
    async handleDelete() {
      ...
      try {
        ...
        Vue.prototype.$toast.success(`Deleted Region ${data.name} successfully!`)
      } catch (error) {
        Vue.prototype.$toast.error(`${error.data.message}`)
        console.error(error)
      } finally {
