  ...
  <v-card class="v-card--material v-card v-sheet theme--light v-card--material--has-heading elevation-2">
    <v-container fluid class="pa-4">
      <v-row class="mb-4" align="center">
        <v-col cols="3">
          <v-tabs v-model="activeTab" dark grow class="rounded-lg elevation-1" background-color="primary lighten-1">
            <v-tab v-for="tab in tabs" :key="tab.name" active-class="active-tab"
        ...
        <v-col cols="4" class="d-flex gap-4">
          <v-autocomplete v-model="filter.region" item-text="name" item-value="name" label="Region" :items="getRegionOptions"
            clearable dense outlined hide-details class="mr-3" @change="handleFilterChange" @click:clear="filter.area = ''">
          ...
          <v-autocomplete v-model="filter.area" item-text="area" item-value="area" label="Area" :disabled="!filter.region"
            :items="filteredAreaOptions" clearable dense outlined hide-details @change="handleFilterChange"
        ...
        <v-col cols="1" class="d-flex justify-end">
          <v-btn fab color="primary" class="" :loading="loading" @click="fetchData">
      ...
      <v-tabs-items v-model="activeTab">
        <v-tab-item>
          <div class="rounded-lg">
            <v-data-table :headers="tableHeaders" :items="tableData" :server-items-length="totalItems"
              :loading="loading" :options.sync="options" hide-default-footer class="small-table"
              @update:options="fetchData">
              <template v-slot:top>
                <v-row class="px-4 py-2" align="center">
                  <v-col cols="4">
                    <v-text-field v-model="search" label="Search" dense outlined hide-details clearable
                      prepend-inner-icon="mdi-magnify" @click:append="handleSearch" />
                  </v-col>
                  <v-spacer></v-spacer>
              ...
              <template v-slot:item="{ item, index }">
                <tr>
                  ...
                  <td>
                    <v-tooltip bottom>
                      <template v-slot:activator="{ on, attrs }">
                        <span v-bind="attrs" class="text-truncate" style="max-width: 150px; display: inline-block;"
                          v-on="on">
                          {{ item?.region }}
                        </span>
                      </template>
                  ...
                  <td>
                    <v-btn small color="primary" class="mr-2" fab @click="openHandleUpdate(item)">
                      <v-icon small>mdi-pencil</v-icon>
                    </v-btn>
                    <v-btn small color="warning" fab @click="handleDetail(item.id)">
                      <v-icon small>mdi-eye</v-icon>
                    </v-btn>
                    <v-btn small color="error" fab @click="openConfirmDeleteDialog(item)">
            ...
            <v-row justify="center" class="py-3">
              <v-pagination v-model="page" :length="totalPages" :total-visible="7" next-icon="mdi-menu-right"
                prev-icon="mdi-menu-left" color="primary" @input="onPageChange" />
            </v-row>
          </div>
        </v-tab-item>
      ...
      <form-survey-new-outlet :dialog="isFormRoleDialog" :is-edit="isEdit" :item="selectedItem" @close="closeFormDialog"
        @save="handleSave" />
    </v-container>
  </v-card>
</template>
