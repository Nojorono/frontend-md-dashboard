<template>
  <v-container fluid>
    <v-card class="v-card--material v-card v-sheet theme--light elevation-4" style="
        padding: 24px;
        border-radius: 16px;
        background: linear-gradient(to right, #ffffff, #f8f9fa);
      ">
      <v-data-table :headers="tableHeaders" :items="tableData" :server-items-length="totalItems" :loading="loading"
        :options.sync="options" hide-default-footer class="" style="border-radius: 8px" @update:options="fetchData">
        <template v-slot:top>
          <v-row class="justify-space-between align-center px-4 py-3">
            <v-col cols="4" class="d-flex align-center">
              <v-autocomplete v-model="filter.region" item-text="name" item-value="name" label="Region"
                :items="getRegionOptions" clearable dense outlined class="mr-4" hide-details
              ...
              <v-autocomplete v-model="filter.area" item-text="area" item-value="area" label="Area"
                :disabled="!filter.region" :items="filteredAreaOptions" clearable dense outlined hide-details
                @change="handleFilterChange">
                <template v-slot:no-data>
                  <v-list-item>
            ...
            <v-col cols="7">
              <div class="d-flex justify-end align-center">
                <v-autocomplete v-model="filter.brand" :items="getBrandOptions" item-text="brand" item-value="brand"
                  label="Brand" clearable dense outlined hide-details class="mr-4" @change="handleFilterChange">
                </v-autocomplete>
                <v-autocomplete v-model="filter.sio_type" :items="getSioTypeOptions" item-text="name" item-value="name"
                  label="Sio Type" class="mr-4" clearable dense outlined hide-details @change="handleFilterChange">
                </v-autocomplete>
                <v-autocomplete v-model="filter.status" :items="statusOptions" label="Status" clearable dense outlined
                  hide-details :item-text="(item) => getStatusLabelOption(item.value)" item-value="value"
                  @change="handleFilterChange">
                  <template v-slot:no-data>
                    <v-list-item>
            ...
            <v-col cols="1" class="d-flex justify-end">
              <v-btn fab color="primary" class="ml-2" :loading="loading" @click="fetchData">
          ...
          <v-row class="justify-space-between align-center px-4 mb-4">
            <v-col cols="3">
              <v-text-field v-model="search" label="Search" dense outlined hide-details clearable
                prepend-inner-icon="mdi-magnify" @keyup.enter="handleSearch" @click:clear="handleClearSearch" />
            </v-col>
          </v-row>
        </template>
        ...
        <template v-slot:item="{ item, index }">
          <tr>
            ...
            <td>
              <v-chip :color="item?.outlet_name ? 'primary' : 'warning'" small label text-color="white">
            ...
            <td>{{ item?.type_sio }}</td>
            <td>
              <v-chip :color="getStatusColor(item?.status)" small label text-color="white">
                {{ getStatusLabel(item) }}
              </v-chip>
            </td>
            <td>
              <v-chip
                :color="item?.status_approval === 0 ? 'warning' : item?.status_approval === 101 ? 'info' : 'success'"
                small label text-color="white">
                <v-icon left small>
                  {{ item?.status_approval === 0 ? 'mdi-alert-circle' : item?.status_approval === 101 ? 'mdi-sync' :
                  'mdi-check-circle' }}
                </v-icon>
                {{ item?.status_approval === 0 ? 'Perlu Verifikasi' : item?.status_approval === 101 ? 'Proses HO' :
                'Approved' }}
              </v-chip>
            </td>
            ...
            <td>
              <div class="d-flex align-center">
                <v-btn color="warning" outlined small class="mr-2" @click="handleDetail(item.id)">
                  ...
                  <v-list dense>
                    ...
                    ">
                      <v-list-item v-for="(status, index) in statusLevel1Options" :key="index"
                        @click="updateStatus(item, status.value)" class="cursor-pointer">
                        <v-list-item-icon class="mr-2">
                          <v-icon small :color="getStatusColor(status.value)">
                            mdi-circle-small
                          </v-icon>
      ...
      <v-row justify="center" class="pt-4">
        <v-pagination v-model="page" :length="totalPages" :total-visible="7" color="primary" next-icon="mdi-menu-right"
          prev-icon="mdi-menu-left" @input="onPageChange" />
      </v-row>
    </v-card>

    <confirm-delete-dialog :dialog="isConfirmDeleteDialogOpen" @confirm="handleDelete"
      @close="closeConfirmDeleteDialog" />

    <form-call-plan :dialog="isFormRoleDialog" :is-edit="isEdit" :item="selectedItem" :refresh="refreshDataTrigger"
      @close="closeFormDialog" @save="handleSave" />
  </v-container>
</template>
