  ...
  <v-app-bar id="app-bar" absolute app color="transparent" flat height="75">
        ...
        <div v-if="notifications.length > 0">
          ...
          >
            ...
            <v-list-item-content>
              <v-list-item-title
                class="subtitle-1"
                :class="{ 'grey--text': notification.is_read }"
              >
                {{
                  notification?.outletExist?.name ||
                  notification?.outletNew?.name
                }}
                -
                {{
                  notification?.outletExist?.outlet_code ||
                  notification?.outletNew?.outlet_code
                }}
              </v-list-item-title>
            ...
            <v-list-item-action v-if="!notification.is_read">
              <v-btn icon small @click.stop="markAsRead(notification.id)">
    ...
    <template>
      <div class="text-center">
        <v-menu offset-y style="max-width: 400px; z-index: 99999999999">
  ...
  methods: {
    ...
    async fetchNotifications() {
      try {
        ...
        const newNotifications = response.data.data.sort(
      ...
      this.$swal
        ...
        .then((result) => {
          if (result.isConfirmed) {
            this.$router.replace({ path: "/logout" }).then(() => {
